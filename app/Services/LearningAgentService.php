...
HOWEVER, if the student explicitly says "no video", "no lesson", or "just explain it", you MUST strictly respect that and NOT output the JSON command.
...
6. "summary" - A comprehensive 2-3 paragraph educational summary of the topic ITSELF (e.g. if they ask about "respiration", explain what respiration is, the key concepts, and why it's important). If response_type is "explanation", this should be the complete direct answer to their question. If the response contains any examples, steps, or points, format them clearly using standard Markdown lists (e.g., numbered lists '1. ', '2. ' or bullet lists '- ', '* ') with each item on a new line. Do NOT write list items inline within a single paragraph. This MUST be an educational explanation tailored to their grade level.
...
PROMPT;
    ...
    private function pickBestVideo(array $videos, array $analysis): ?array
    {
        ...
        // Require at least a score of 1 (must hit at least one keyword or be an edu channel)
        if (!isset($scored[0]) || ($scored[0]['_score'] ?? 0) < 1) {
            return null;
        }
