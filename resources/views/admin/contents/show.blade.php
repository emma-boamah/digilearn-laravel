...
<div class="min-h-screen bg-slate-50/60 dark:bg-gray-900 py-6 px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.contents.index') }}" 
               class="inline-flex items-center justify-center w-9 h-9 rounded-xl bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 text-slate-600 dark:text-gray-300 hover:text-blue-600 hover:border-blue-300 transition-all shadow-xs">
                <i class="fas fa-arrow-left text-sm"></i>
            </a>
            <div>
                <div class="flex items-center gap-2 flex-wrap">
                    <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Content Inspection</span>
                    <span class="text-slate-300">•</span>
                    <span class="text-xs font-medium text-slate-500">ID #{{ $content->id }}</span>
                    @if($contentType === 'video')
                        <span class="px-2 py-0.5 rounded-full text-[11px] font-bold bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300">
                            <i class="fas fa-video mr-1"></i>Video Package
                        </span>
                    @elseif($contentType === 'document')
                        <span class="px-2 py-0.5 rounded-full text-[11px] font-bold bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300">
                            <i class="fas fa-file-alt mr-1"></i>Document
                        </span>
                    @elseif($contentType === 'quiz')
                        <span class="px-2 py-0.5 rounded-full text-[11px] font-bold bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300">
                            <i class="fas fa-clipboard-check mr-1"></i>Quiz
                        </span>
                    @endif

                    @if(isset($content->is_agent_generated) && $content->is_agent_generated)
                        <span class="px-2 py-0.5 rounded-full text-[11px] font-bold bg-amber-100 dark:bg-amber-900/40 text-amber-800 dark:text-amber-300 flex items-center gap-1">
                ...
                <h1 class="text-xl sm:text-2xl font-black text-slate-900 dark:text-white mt-1 line-clamp-1">
                    {{ $content->title }}
                </h1>
            </div>
        </div>
        ...
        <div class="flex items-center gap-2.5 flex-wrap">
            <a href="{{ route('admin.contents.edit', $content->id) }}" 
               class="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs sm:text-sm font-bold rounded-xl shadow-xs transition-colors">
                <i class="fas fa-edit"></i>
                Edit Content
            </a>
            <a href="{{ route('admin.contents.index') }}" 
               class="inline-flex items-center gap-2 px-3.5 py-2 bg-white dark:bg-gray-800 hover:bg-slate-50 dark:hover:bg-gray-700 text-slate-700 dark:text-gray-300 text-xs sm:text-sm font-semibold rounded-xl border border-slate-200 dark:border-gray-700 transition-colors shadow-xs">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-xs overflow-hidden">
                    ...
                    <div class="aspect-video bg-slate-950 flex items-center justify-center relative group">
                        ...
                        @else
                            <div class="text-center p-8 text-slate-400">
                                ...
                                <h3 class="text-sm font-bold text-slate-300">No Video Stream Configured</h3>
                                <p class="text-xs text-slate-500 mt-1 max-w-sm">This video package does not currently have an active video stream ID or local video file attached.</p>
                            </div>
                        @endif
                    </div>
                @elseif($contentType === 'document')
                    <div class="p-8 bg-gradient-to-br from-emerald-900/10 to-teal-900/5 dark:from-emerald-950/30 dark:to-gray-900 border-b border-slate-100 dark:border-gray-700 flex flex-col sm:flex-row items-center gap-6">
                        ...
                        <div class="space-y-2 text-center sm:text-left">
                            <span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300">
                                Standalone Document
                            </span>
                            <h2 class="text-lg font-bold text-slate-900 dark:text-white">{{ $content->title }}</h2>
                            <p class="text-xs text-slate-500 font-mono">{{ $content->file_path ?? 'No file path stored' }}</p>
                            @if(!empty($content->file_path))
                                <div class="pt-2">
                                    <a href="{{ asset('storage/' . $content->file_path) }}" target="_blank" download
                                       class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold transition-colors">
                ...
                @elseif($contentType === 'quiz')
                    <div class="p-8 bg-gradient-to-br from-purple-900/10 to-indigo-900/5 dark:from-purple-950/30 dark:to-gray-900 border-b border-slate-100 dark:border-gray-700 flex flex-col sm:flex-row items-center gap-6">
                        ...
                        <div class="space-y-2 text-center sm:text-left">
                            <span class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300">
                                Assessment / Quiz Hub
                            </span>
                            <h2 class="text-lg font-bold text-slate-900 dark:text-white">{{ $content->title }}</h2>
                ...
                <!-- Content Description -->
                <div class="p-6">
                    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-2">Description & Notes</h3>
                    @if(!empty($content->description))
                        <div class="text-sm text-slate-700 dark:text-gray-300 leading-relaxed bg-slate-50 dark:bg-gray-900/50 p-4 rounded-xl border border-slate-100 dark:border-gray-700/60">
                            {!! nl2br(e($content->description)) !!}
                        </div>
                    @else
                        <p class="text-xs italic text-slate-400">No written description provided for this content.</p>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-xs p-6">
                <div class="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-gray-700 mb-4">
                    <div class="flex items-center gap-2.5">
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-slate-900 dark:text-white">Media & File Storage Telemetry</h3>
                            <p class="text-[11px] text-slate-400">Hosting destinations, raw storage paths, and media parameters</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                    <!-- Source Platform -->
                    <div class="p-3.5 bg-slate-50 dark:bg-gray-900/40 rounded-xl border border-slate-100 dark:border-gray-700">
                        <span class="text-[11px] font-semibold text-slate-400 block mb-1">Primary Source Provider</span>
                        <span class="font-bold text-slate-800 dark:text-gray-200 capitalize flex items-center gap-1.5">
                            @if(($content->video_source ?? '') === 'vimeo' || !empty($content->vimeo_id))
                                <i class="fab fa-vimeo text-blue-500 text-sm"></i> Vimeo Cloud Hosting
                            @elseif(($content->video_source ?? '') === 'youtube' || !empty($content->external_video_id))
                                <i class="fab fa-youtube text-red-500 text-sm"></i> YouTube Embed
                            @elseif(($content->video_source ?? '') === 'mux' || !empty($content->mux_playback_id))
                                <i class="fas fa-play-circle text-purple-500 text-sm"></i> Mux Video Streaming
                            @elseif(!empty($content->video_path) || !empty($content->temp_file_path))
                                <i class="fas fa-hdd text-emerald-500 text-sm"></i> Local Server Storage
                            @else
                                <i class="fas fa-file text-slate-400 text-sm"></i> {{ ucfirst($content->video_source ?? 'Direct File') }}
                            @endif
                        </span>
                    </div>
                    ...
                    <!-- Duration / Size -->
                    <div class="p-3.5 bg-slate-50 dark:bg-gray-900/40 rounded-xl border border-slate-100 dark:border-gray-700">
                        <span class="text-[11px] font-semibold text-slate-400 block mb-1">Media Duration / Specs</span>
                    ...
                    <div class="p-3.5 bg-slate-50 dark:bg-gray-900/40 rounded-xl border border-slate-100 dark:border-gray-700 sm:col-span-2">
                        <span class="text-[11px] font-semibold text-slate-400 block mb-1">Raw File Path / Remote ID</span>
                        <div class="font-mono text-slate-700 dark:text-gray-300 break-all bg-white dark:bg-gray-800 p-2 rounded-lg border border-slate-200 dark:border-gray-700 text-[11px]">
                            @if(!empty($content->vimeo_id))
                    ...
                    @if(!empty($content->thumbnail_path) || !empty($content->thumbnail_url))
                        <div class="p-3.5 bg-slate-50 dark:bg-gray-900/40 rounded-xl border border-slate-100 dark:border-gray-700 sm:col-span-2">
                            <span class="text-[11px] font-semibold text-slate-400 block mb-1">Cover Thumbnail Storage Destination</span>
                            <div class="font-mono text-slate-700 dark:text-gray-300 break-all bg-white dark:bg-gray-800 p-2 rounded-lg border border-slate-200 dark:border-gray-700 text-[11px]">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-xs p-6">
                    <div class="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-gray-700 mb-4">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                <h3 class="text-sm font-bold text-slate-900 dark:text-white">Attached Study Documents & Worksheets</h3>
                                <p class="text-[11px] text-slate-400">Downloadable learning material linked to this lesson</p>
                            </div>
                        </div>
                            ...
                            @foreach($content->documents as $doc)
                                <div class="p-4 bg-slate-50 dark:bg-gray-900/40 rounded-xl border border-slate-100 dark:border-gray-700 flex items-center justify-between gap-4">
                                    <div class="flex items-center gap-3 min-w-0">
                                        ...
                                        <div class="min-w-0">
                                            <h4 class="text-xs font-bold text-slate-800 dark:text-gray-200 truncate">{{ $doc->title }}</h4>
                                            <p class="text-[11px] font-mono text-slate-400 truncate">storage/app/public/{{ $doc->file_path }}</p>
                                        </div>
                                    </div>
                                    @if(!empty($doc->file_path))
                                        <a href="{{ asset('storage/' . $doc->file_path) }}" target="_blank" download
                                           class="px-3 py-1.5 rounded-lg bg-white dark:bg-gray-800 border border-slate-200 dark:border-gray-700 text-slate-700 dark:text-gray-300 hover:bg-emerald-50 hover:text-emerald-700 text-xs font-bold transition-colors flex items-center gap-1.5 flex-shrink-0 shadow-2xs">
                                            <i class="fas fa-download"></i>
                                            Download
                                        </a>
                                    @endif
                        ...
                        @elseif(!empty($content->document_path))
                            <div class="p-4 bg-slate-50 dark:bg-gray-900/40 rounded-xl border border-slate-100 dark:border-gray-700 flex items-center justify-between gap-4">
                                <div class="flex items-center gap-3 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                        <h4 class="text-xs font-bold text-slate-800 dark:text-gray-200 truncate">Primary PDF Document</h4>
                                        <p class="text-[11px] font-mono text-slate-400 truncate">storage/app/public/{{ $content->document_path }}</p>
                                    </div>
                                </div>
            ...
            @if($quizModel && $quizData && isset($quizData['questions']) && count($quizData['questions']) > 0)
                <div class="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-xs p-6">
                    <div class="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-gray-700 mb-4">
                        <div class="flex items-center gap-2.5">
                            <div class="w-8 h-8 rounded-lg bg-purple-50 dark:bg-purple-900/40 text-purple-600 dark:text-purple-400 flex items-center justify-center text-sm font-bold">
                                <i class="fas fa-question-circle"></i>
                            </div>
                            <div>
                                <h3 class="text-sm font-bold text-slate-900 dark:text-white">Connected Quiz Questions & Answers</h3>
                                <p class="text-[11px] text-slate-400">Structure of assessments attached to this lesson package</p>
                            </div>
                        </div>
                    ...
                    <div class="space-y-4">
                        @foreach($quizData['questions'] as $qIdx => $q)
                            ...
                            @endphp
                            <div class="p-4 bg-slate-50/80 dark:bg-gray-900/50 rounded-xl border border-slate-200 dark:border-gray-700">
                                <div class="flex items-start justify-between gap-3 mb-2.5">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-extrabold uppercase {{ $qType === 'essay' ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800' }}">
                                ...
                                @if($qType === 'mcq' && is_array($options) && count($options) > 0)
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                        @foreach($options as $optIdx => $opt)
                                            ...
                                            @endphp
                                            <div class="p-2 rounded-lg text-xs flex items-center gap-2 border {{ $isCorrect ? 'bg-emerald-50 border-emerald-300 text-emerald-900 font-semibold' : 'bg-white dark:bg-gray-800 border-slate-200 dark:border-gray-700 text-slate-600 dark:text-gray-300' }}">
                                ...
                                @elseif($qType === 'essay')
                                    <div class="p-3 bg-amber-50/50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/40 rounded-lg text-xs text-amber-900 dark:text-amber-300">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-xs p-6">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white pb-3 border-b border-slate-100 dark:border-gray-700 mb-4 flex items-center gap-2">
                ...
                <div class="space-y-3.5 text-xs">
                    <!-- Subject -->
                    <div>
                        <span class="text-slate-400 block text-[11px] font-semibold uppercase mb-1">Subject</span>
                        <div class="font-bold text-slate-800 dark:text-gray-200 text-sm flex items-center gap-2">
                    ...
                    <!-- Grade Level -->
                    <div>
                        <span class="text-slate-400 block text-[11px] font-semibold uppercase mb-1">Grade Level</span>
                        <span class="font-bold text-slate-800 dark:text-gray-200 bg-slate-100 dark:bg-gray-700 px-2.5 py-1 rounded-lg">
                    ...
                    <div>
                        <span class="text-slate-400 block text-[11px] font-semibold uppercase mb-1">Level Group Tier</span>
                        <div class="font-semibold text-slate-700 dark:text-gray-300">
                            @if($levelGroup)
                                <span class="px-2.5 py-1 rounded-lg bg-indigo-50 text-indigo-700 font-bold border border-indigo-100">
                                    <i class="fas fa-layer-group mr-1"></i>{{ $levelGroup->title }}
                                </span>
                            @else
                                <span class="text-slate-400 italic">No specific Level Group bound</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div>
                        <span class="text-slate-400 block text-[11px] font-semibold uppercase mb-1.5">Categories & Context Tags</span>
                        <div class="flex flex-wrap gap-1.5">
                            @if(isset($content->categories) && $content->categories->count() > 0)
                                ...
                                @endforeach
                            @else
                                <span class="text-slate-400 italic text-[11px]">Normal Curriculum</span>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-xs p-6">
                ...
                @if($content->uploader)
                    <div class="flex items-center gap-3.5 mb-4">
                        ...
                        <div class="min-w-0">
                            <h4 class="text-sm font-bold text-slate-900 dark:text-white truncate">{{ $content->uploader->name }}</h4>
                            <p class="text-xs text-slate-500 truncate">{{ $content->uploader->email }}</p>
                            @if($content->uploader->roles && $content->uploader->roles->count() > 0)
                                <div class="mt-1 flex flex-wrap gap-1">
                                    @foreach($content->uploader->roles as $role)
                                        <span class="px-1.5 py-0.2 rounded text-[10px] font-bold uppercase bg-slate-100 text-slate-700">
                                            {{ $role->name }}
                                        </span>
                                    @endforeach
                                </div>
                ...
                @else
                    <div class="flex items-center gap-3 text-slate-500 mb-4">
                        ...
                        <div>
                            <span class="text-xs font-bold">{{ $content->uploader_name ?? 'System Administrator' }}</span>
                            <p class="text-[11px] text-slate-400">Automated / System</p>
                        </div>
                    </div>
                @endif

                <div class="pt-3 border-t border-slate-100 dark:border-gray-700 space-y-2 text-[11px] text-slate-500">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-slate-200 dark:border-gray-700 shadow-xs p-6">
                ...
                <div class="grid grid-cols-2 gap-3 text-center">
                    <div class="p-3 bg-blue-50/50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-800">
                        <span class="text-xl font-black text-blue-700 dark:text-blue-300 block">
                            {{ number_format($content->views ?? 0) }}
                        </span>
                        <span class="text-[10px] uppercase font-bold text-blue-600/80">Lifetime Views</span>
                    </div>

                    <div class="p-3 bg-emerald-50/50 dark:bg-emerald-900/20 rounded-xl border border-emerald-100 dark:border-emerald-800">
                        <span class="text-xl font-black text-emerald-700 dark:text-emerald-300 block">
                            {{ number_format($content->comments_count ?? 0) }}
                        </span>
                        <span class="text-[10px] uppercase font-bold text-emerald-600/80">Discussions</span>
                    </div>
                </div>
            </div>
