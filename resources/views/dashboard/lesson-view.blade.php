...
<style nonce="{{ request()->attributes->get('csp_nonce') }}">
    ...
    .right-sidebar {
        ...
        width: 100%;
        min-width: 0;
        /* changed from 300px to prevent overflow */
        max-width: 400px;
    }
    ...
    .mobile-video-badge {
        display: none;
        pointer-events: none;
        /* Prevents badge from blocking taps on the video */
        opacity: 1;
    ...
    @media (max-width: 768px) {
        * {
        ...
        .lesson-sidebar {
            display: contents;
            /* Children become direct grid children */
            width: 100%;
            min-width: unset;
            max-width: 100%;
        }
...
<div class="lesson-page">
                ...
                <div class="course-tabs">
                    ...
                    <div class="tab-content active" id="videos-tab">
                        ...
                        <div class="course-item video-item">
                            <div class="item-thumbnail">
                                <video class="item-video" muted preload="metadata"
                                    poster="{{ secure_asset($video->getThumbnailUrl()) }}">
                                ...
                                <div class="item-duration">{{ $video->duration_seconds ? floor($video->duration_seconds
                                    / 60) . ':' . str_pad($video->duration_seconds % 60, 2, '0', STR_PAD_LEFT) : 'N/A'
                                    }}</div>
                            </div>
                            <div class="item-info">
                    ...
                    <div class="tab-content" id="documents-tab">
                        ...
                        <div class="course-item document-item">
                            ...
                            <div class="item-actions">
                                <a href="{{ route('dashboard.lesson.document', ['lessonId' => \App\Services\UrlObfuscator::encode($document->id), 'type' => 'pdf']) }}"
                    ...
                    <div class="tab-content" id="quizzes-tab">
                        ...
                        <div class="course-item quiz-item">
                            ...
                            <div class="item-actions">
                                <a href="{{ route('quiz.instructions', ['quizId' => \App\Services\UrlObfuscator::encode($quiz->id)]) }}"
                                    class="action-btn-small primary">
                                    <svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24">
                                        <path
                                            d="M14.23 12.004a2.236 2.236 0 0 0 .53-1.482 2.236 2.236 0 0 0-.53-1.482l-4.23-2.882a2.236 2.236 0 0 0-2.46 0L3.31 9.04a2.236 2.236 0 0 0 0 3.848l4.23 2.882a2.236 2.236 0 0 0 2.46 0l4.23-2.882z" />
                                    </svg>
                                    Take Quiz
                                </a>
                            </div>
    ...
    <aside class="lesson-sidebar">
        ...
        <div class="action-buttons-grid">
            ...
            <button class="action-btn action-navigate-btn"
                data-href="{{ route('dashboard.lesson.document', ['lessonId' => \App\Services\UrlObfuscator::encode($lesson['id']), 'type' => 'pdf']) }}"
            ...
            <button class="action-btn action-navigate-btn"
                data-href="{{ route('dashboard.lesson.document', ['lessonId' => \App\Services\UrlObfuscator::encode($lesson['id']), 'type' => 'ppt']) }}"
                data-resource-type="ppt">
                <svg fill="currentColor" viewBox="0 0 24 24">
            ...
            <div class="notes-title-modal">
                ...
                <div class="notes-title-modal-body">
                    ...
                    <div class="notes-title-input-group">
                        <input type="text" class="notes-title-input" id="notesTitleInput"
                            placeholder="Enter notes title..." maxlength="255">
                        <div class="notes-title-suggestions">
                            <button class="notes-title-suggestion-btn"
                                data-title="{{ Str::slug($lesson['title'] ?? '') }}">
        ...
        <div class="related-videos-card">
            ...
            @include('dashboard.partials.related-lesson-card', ['lesson' => $relatedLesson])
            @endforeach
            @else
            <!-- Sample related videos for demo -->
            @for($i = 1; $i <= 8; $i++) @php $demoLesson=[ 'id'=> "demo-{$i}",
                'title' => 'Living and non-living organisms',
                'subject' => 'Science',
                ...
                @endphp
                @include('dashboard.partials.related-lesson-card', ['lesson' => $demoLesson])
                @endfor
                @endif
        </div>
</div>
        ...
        <div class="notes-education-modal-body">
            ...
            <div class="notes-education-modal-options">
                <div class="notes-education-option">
                    ...
                    <p>Completely replace your current notes with new content. Use this when you want to start fresh.
                    </p>
                </div>
                ...
                <div class="notes-education-option">
                    ...
                    <p>Append new content to your existing notes. Perfect for adding more thoughts or continuing where
                        you left off.</p>
                </div>
            </div>
            ...
            <div class="notes-education-modal-tip">
                ...
                <span><strong>Pro tip:</strong> You can always edit your notes later, and this choice only affects how
                    new content is added. Use <kbd>Ctrl+S</kbd> to quickly save!</span>
            </div>
        </div>
...
<script nonce="{{ request()->attributes->get('csp_nonce') }}">
        ...
        document.querySelectorAll('.related-video-item').forEach(item => {
            item.addEventListener('click', function (e) {
                if (e.target.closest('.action-icon-btn')) {
                    return; // Let the global icon handler in digilearn.blade.php deal with it
                }
    ...
    function initializeRelatedSaveButtons() {
        document.addEventListener('click', async function (e) {
            const saveBtn = e.target.closest('.action-icon-btn.save-btn');
            if (!saveBtn) return;
            ...
            const lessonId = saveBtn.dataset.lessonId;
            if (!lessonId) return;
            ...
            const url = isSaved ? `/dashboard/lesson/${lessonId}/unsave` : `/dashboard/lesson/${lessonId}/save`;
            const method = isSaved ? 'DELETE' : 'POST';

            const lessonData = isSaved ? {} : {
                lesson_title: saveBtn.dataset.title || (card ? card.querySelector('.video-title').textContent : 'Untitled'),
                lesson_subject: saveBtn.dataset.subject || 'General',
                lesson_instructor: saveBtn.dataset.instructor || 'Unknown',
                lesson_year: saveBtn.dataset.year || new Date().getFullYear().toString(),
                lesson_thumbnail: saveBtn.dataset.thumbnail || '',
                lesson_duration: saveBtn.dataset.duration || '0:00',
                lesson_video_url: saveBtn.dataset.videoUrl || '',
                selected_level: saveBtn.dataset.selectedLevel || 'primary-lower'
            };
            ...
            try {
                ...
                if (data.success) {
                    ...
                    saveBtn.innerHTML = data.saved ? '<svg width="20" height="20" fill="currentColor" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" class="text-secondary-blue"><path d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"/></svg>' : '<svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"/></svg>';
                    if (window.showSuccessMessage) window.showSuccessMessage(data.message);
                } else {
                    saveBtn.innerHTML = originalHTML;
        ...
        // On load, fetch status of all visible save buttons attached to related videos
        document.querySelectorAll('.action-icon-btn.save-btn').forEach(btn => {
            const id = btn.dataset.lessonId;
            if (!id) return;
            fetch(`/dashboard/lesson/${id}/check-saved`, {
                method: 'GET',
                headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' }
            }).then(res => res.json()).then(data => {
                if (data.saved) {
                    btn.dataset.saved = 'true';
                    btn.innerHTML = '<svg width="20" height="20" fill="currentColor" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" class="text-secondary-blue"><path d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"/></svg>';
                }
